...
function load() {
	let get1 = localStorage.getItem(modInfo.id);

	if (get1 === null || get1 === undefined) {
		let getold = localStorage.getItem(modInfo.lastid);
		if (getold === null || getold === undefined) {
			player = getStartPlayer();
			options = getStartOptions();
		}
		else {
			player = Object.assign(getStartPlayer(), JSON.parse(decodeURIComponent(escape(atob(getold)))));
			localStorage.removeItem(modInfo.lastid)
			fixSave();
			loadOptions();
		}
	}
	else {
		player = Object.assign(getStartPlayer(), JSON.parse(decodeURIComponent(escape(atob(get1)))));
		fixSave();
		loadOptions();
	}
...
function loadOptions() {
	...
	if (get2) 
		...
		let getold2 = localStorage.getItem(modInfo.lastid+"_options");
		if (getold2) {
			options = Object.assign(getStartOptions(), JSON.parse(decodeURIComponent(escape(atob(getold2)))));
			localStorage.removeItem(modInfo.lastid+"_options")
		}
...
function setupModInfo() {
	...
	modInfo.winText = winText ? winText : `恭喜！你已经走到了游戏的终点并取得了胜利，但是现在……`;
	...
	for (item in data) {
		if (data[item] == null) {
			...
			if (!NaNalert) {
				...
				NaNalert = true;
				alert("在游戏中发现了无效的值，名为 \"" + item + "\"。请告诉该游戏的创作者！你可以刷新这个界面，然后继续游玩")
				return
			}
		}
...
function importSave(imported = undefined, forced = false) {
	...
	try {
		...
		if (tempPlr.versionType != modInfo.id && !forced && !confirm("这个存档似乎是其它的模组树！您确定要导入吗？")) // Wrong save (use "Forced" to force it to accept.)
			return;
		player = tempPlr;
