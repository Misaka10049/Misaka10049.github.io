var systemComponents = {
	...
	'tree-node': {
      ...
      v-if="tmp[layer].tooltip != ''"
			:text="(tmp[layer].isLayer) ? (
				...
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : '得到 ' + formatWhole(tmp[layer].requires) + ' ' + tmp[layer].baseResource + ' 去解锁 (你有 ' + formatWhole(tmp[layer].baseAmount) + ' ' + tmp[layer].baseResource + ')')
			)
			: (
				tmp[layer].canClick ? (tmp[layer].tooltip ? tmp[layer].tooltip : '一个按钮')
				: (tmp[layer].tooltipLocked ? tmp[layer].tooltipLocked : '一个按钮')
			)"></tooltip>
    ...
    'info-tab': {
        ...
        <span v-if="modInfo.author">
            <br><br>
            制作 by {{modInfo.author}}	
        </span>
        <br>
        模组树 <a v-bind:href="'https://github.com/Acamaeda/The-Modding-Tree/blob/master/changelog.md'" target="_blank" class="link" v-bind:style = "{'font-size': '14px', 'display': 'inline'}" >v{{TMT_VERSION.tmtNum}}</a> by Acamaeda
        <br>
        声望树 by Jacorb and Aarex
		<br><br>
		...
		<br><br>
        已玩时间: {{ formatTime(player.timePlayed) }}<br><br>
        <h3>快捷键</h3><br>
    ...
    'options-tab': {
        ...
        <table>
            ...
            <tr>
                <td><button class="opt" onclick="exportSave()">导出存档到剪切板</button></td>
                <td><button class="opt" onclick="importSave()">导入存档</button></td>
                <td><button class="opt" onclick="toggleOpt('offlineProd')">计算离线收益: {{ options.offlineProd?"开":"关" }}</button></td>
            </tr>
            <tr>
                <td><button class="opt" onclick="switchTheme()">主题: {{ translate(getThemeName()) }}</button></td>
                <td><button class="opt" onclick="adjustMSDisp()">展示里程碑: {{ translate(MS_DISPLAYS[MS_SETTINGS.indexOf(options.msDisplay)])}}</button></td>
                <td><button class="opt" onclick="toggleOpt('hqTree')">高质量的树节点贴图: {{ options.hqTree?"开":"关" }}</button></td>
            </tr>
            <tr>
                <td><button class="opt" onclick="toggleOpt('hideChallenges')">已完成的成就: {{ options.hideChallenges?"隐藏":"显示" }}</button></td>
                <td><button class="opt" onclick="toggleOpt('forceOneTab'); needsCanvasUpdate = true">单标签页面: {{ options.forceOneTab?"一直":"自动" }}</button></td>
			</tr> 
        </table>`
    },
